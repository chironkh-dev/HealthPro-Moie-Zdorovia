...
## Початок сесії — обов'язковий чеклист

> Агент виконує цей чеклист на початку **кожної** нової розмови.

1. **Запитати версію:** "Яка поточна версія додатку?" — без цього не формувати changelog і не генерувати звіт.
2. **Прочитати `replit.md`** — переконатись у знанні актуального стану проєкту.
3. **Уточнити задачу** — якщо запит неоднозначний, задати одне уточнююче питання.
4. **Після роботи** — оновити `## Changelog` у `replit.md` новим записом та `## Gotchas` якщо знайдено нові підводні камені.

---

## Changelog

### v5.3.29 (2026-05-21) — Сесія: UI Bugfix 10 пунктів

- **1а — Кнопки модалі звіту:** `btn-print` → `btn btn-blue`; "Поділитись" — зелений градієнт. Іконки тепер білі і добре видно.
- **1б — shareReport виправлено:** видалено `_platformShare({files:[Blob]})` (Capacitor Share не приймає Blob). Тепер `platformDownload` — записує файл → Share Sheet з URI. Файл: `src/features/export/index.js`.
- **2а — PDF page break:** після html2canvas — вимірюються позиції `.pdf-section-head` елементів; якщо заголовок секції в останніх 22 мм сторінки → біла заливка + новий аркуш починається з заголовка. Файл: `pdf-report.js`.
- **2б — Заголовки PDF:** видалено "(останні 30)" з `journalTitle` та "— 30 днів" з `adherTitle` (uk + ru). Файл: `i18n/pdf.js`.
- **2в — Пульс на графіку PDF:** чекбокс "Пульс на графіку" у секціях звіту. `buildBPChartSVG(data, showPulse)` — помаранчева пунктирна лінія + легенда. Файли: `index.html`, `pdf-report.js`, `export/modal.js`.
- **2г — Логотип у PDF:** inline SVG-іконка ЕКГ-хвилі на синьому квадраті в шапці PDF. Файл: `pdf-report.js`.
- **3а — "Експорт CSV":** рядок видалено з блоку "Резервна копія" в `index.html`.
- **4а — Кнопка "Додати препарат":** `btn-violet` → `btn-blue`.
- **4б — Застереження:** кнопка "? Застереження" → абсолютно-позиційований SVG-трикутник (червоний, "!") у правому верхньому куті картки. Картка отримала `position:relative`.
- **4в — Одиниця дозування:** `<select id="pillUnit">` (мг/мкг/мл/шт./крап.) поруч з полем дозування. `addPill()` зберігає "50 мг". PDF та список "Усі препарати" відображають повний рядок.
