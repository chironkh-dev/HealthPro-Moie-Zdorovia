...
- `HealthPro-Moie-Zdorovia/src/core/charts.js` — **ECharts factory** (tree-shaking: `init`, `use`, `SVGRenderer`, `CanvasRenderer`). WeakMap інстансів, `createChart(el, renderer)` / `disposeChart(el)`.
- `HealthPro-Moie-Zdorovia/src/features/analytics/iz-chart.js` — ІЗ-тренд (30 днів): LineChart + markLine 60/80 + кольорові точки за зонами.
- `HealthPro-Moie-Zdorovia/src/features/analytics/trend-modal.js` — модал тиску (sys+dia): LineChart ECharts замість raw Canvas2D.
- vite.config `manualChunks`: `echarts` → окремий chunk (≈1.1 МБ gzip).
