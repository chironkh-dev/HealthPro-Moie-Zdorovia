export const T_UK ={
  'nt-pressure':'Тиск','nt-analytics':'Аналіз','nt-pills':'Ліки','nt-history':'Журнал','nt-profile':'Профіль','nt-settings':'Налашт.',
  't-install':'Встановіть додаток на телефон!','t-install-btn':'Встановити','t-update':'Доступне оновлення','t-update-btn':'Оновити','t-offline':'Офлайн режим — дані зберігаються локально',
  't-last-bp':'Останній тиск','t-last-pulse':'Пульс','t-bpm':'уд/хв','t-new-meas':'Новий вимір',
  't-bp-label':'Тиск (sys / dia) та Пульс','t-note':'Нотатка','t-save-meas':'Зберегти вимір',
  't-chart-title':'Графік вимірів','t-chart-all':'Все','t-pulse-toggle':'Пульс',
  't-leg-sys':'Систол.','t-leg-dia':'Діастол.',
  't-health-index':'Індекс здоров\'я','t-iz-trend-title':'Тренд Індексу Здоров\'я (30 днів)','t-avg7':'Середній тиск (7 вим.)','t-trend':'Тенденція','t-tap-graph':'▲ Натисни — графік',
  't-who-cat':'Категорія тиску (ESC 2024)','t-who-cat-aha':'Категорія тиску (AHA 2017)','t-total-m':'Всього вимірів','t-adh':'Прийом ліків','t-today':'за сьогодні',
  't-maxmin':'Макс. / Мін.','t-avg-pulse':'Середній пульс','t-bpm2':'уд/хв · норма 60–100','t-pct-norm':'% норм. вимірів','t-of-total':'від загальних',
  't-bmi-title':'ІМТ та вплив на тиск','t-bmi-empty':'Заповніть зріст та вагу в Профілі',
  't-steps-title':'Активність сьогодні','t-steps':'кроків сьогодні','t-step-goal':'ціль: 10 000','t-recos':'Рекомендації','t-reco-empty':'Додайте виміри',
  't-cnt-total':'Всього','t-cnt-taken':'Прийнято','t-cnt-left':'Залишилось','t-adh2':'Виконання сьогодні',
  't-today-pills':'Сьогодні','t-no-pills':'Немає ліків на сьогодні','t-all-pills':'Усі препарати',
  't-add-pill':'Додати препарат','t-pill-name':'Назва препарату *','t-pill-dose':'Дозування *','t-pill-time':'Час прийому',
  't-pill-days':'Дні прийому','t-pill-date':'Дата','t-add-pill-btn':'Додати препарат','t-pharmacy':'Пошук в аптеках',
  't-discl-title':'Застереження','t-discl-text':'Інформація про препарати носить виключно інформаційний характер. Не є медичною порадою. Перед застосуванням будь-якого препарату проконсультуйтеся з лікарем. Самолікування може бути небезпечним для вашого здоров\'я.',
  't-journal':'Журнал вимірів','t-h-week':'Тиждень','t-h-month':'Місяць','t-h-all':'Всі','t-no-history':'Немає записів','t-clear-hist':'Очистити журнал',
  't-profile':'Мій профіль','t-name':'Ім\'я','t-age':'Вік','t-height':'Зріст (см)','t-weight':'Вага (кг)','t-gender':'Стать','t-phone':'Телефон','t-email':'Email',
  't-save-profile':'Зберегти профіль','t-emergency':'Екстрений контакт','t-e-phone':'Телефон','t-e-name':'Ім\'я контакту',
  't-emergency-info':'При тиску ≥180/120 з\'явиться кнопка SMS-сповіщення цьому контакту.','t-test-sms':'Тест SMS',
  't-notif':'Нагадування','t-notif-pill':'Нагадування про ліки','t-notif-pill-sub':'Push при настанні часу',
  't-measure-remind':'Нагадування про вимір','t-measure-remind-sub':'Ранок та вечір (налаштовуйте час)',
  't-morning':'Ранок','t-evening':'Вечір',
  't-push-note':'Нагадування працюють навіть при вимкненому додатку (на смартфоні через системні сповіщення).',
  't-lang-title':'Мова / Язык','t-pedometer':'Лічильник кроків','t-step-enable':'Відстеження активності','t-step-sub':'Через сенсори телефону',
  't-step-goal-lbl':'Денна ціль (кроків)','t-backup':'Резервна копія','t-exp-json':'Експорт JSON','t-exp-json-sub':'Повний бекап',
  't-exp-csv':'Експорт CSV','t-exp-csv-sub':'Для Excel / лікаря','t-exp-pdf':'Друк / PDF','t-exp-pdf-sub':'Звіт через друк',
  't-imp':'Імпорт даних','t-imp-sub':'Відновити з JSON','t-imp-btn':'Відновити',
  't-email-notif':'Нагадування на Email/SMS','t-email-info':'Швидко надішліть собі Email або SMS-нагадування. Адреса та номер беруться з Профілю.',
  't-about':'Про додаток','t-version':'Версія','t-data':'Дані','t-data-sub':'Зберігаються тільки на вашому пристрої',
  't-disclaimer':'⚠️ Додаток не замінює консультацію лікаря. При погіршенні — зверніться до медика.','t-view-discl':'Переглянути медичний дисклеймер','t-delete-all':'Видалити всі дані',
  't-trend-modal':'Тенденція тиску','t-who-modal':'Класифікація тиску ВООЗ',
  't-health-tooltip-title':'Деталі індексу','t-health-tooltip-ok':'Зрозуміло',
  't-score-bp-lbl':'Тиск:','t-score-pulse-lbl':'Пульс:','t-score-pills-lbl':'Ліки:','t-score-activity-lbl':'Активність:','t-score-bmi-lbl':'ІМТ:',
  't-veto-label':'ВЕТО:','t-veto-text':'Ваш останній тиск занадто високий. Загальний індекс автоматично знижено для вашої безпеки.',
  'o-daily':'Щодня','o-date':'Дата',
  'o-gender-none':'Не вказано','o-male':'Чоловіча','o-female':'Жіноча',
  't-score-bal':'бал',
  't-score-goal':'Ціль: 80+',
  't-settings-doc-title':'Звіт для лікаря',
  't-settings-doc-label':'Друк / PDF / CSV',
  't-settings-doc-sub':'Оберіть період та формат звіту',
  't-settings-doc-btn':'Звіт',
  't-export-title':'Звіт для лікаря',
  't-export-period-lbl':'Вибір периоду',
  't-exp-btn-week':'Тиждень',
  't-exp-btn-2weeks':'2 тижні',
  't-exp-btn-month':'Місяць',
  't-exp-btn-quarter':'90 днів',
  't-exp-btn-custom':'Свій період',
  't-exp-format-lbl':'Формат',
  't-exp-format-pdf':'PDF',
  't-exp-format-csv':'CSV',
  't-exp-generate-btn':'Зберегти звіт',
  't-exp-share-btn':'Поділитись',
  't-exp-share-dialog':'Надіслати звіт лікарю',
  'e-share-report-done':'✓ Звіт відправлено!',
  'e-csv-patient-lbl':'=== ДАНІ ПАЦІЄНТА ===',
  'e-csv-bp-lbl':'=== ВИМІРИ ТИСКУ ===',
  'e-csv-meds-lbl':'=== ЛІКИ ===',
  'e-csv-adher-lbl':'=== ПРИЙОМ ЛІКІВ ===',
  'e-csv-col-name':'ПІБ',
  'e-csv-col-age':'Вік',
  'e-csv-col-gender':'Стать',
  'e-csv-male':'Чоловіча',
  'e-csv-female':'Жіноча',
  'e-csv-col-height-weight':'Зріст / Вага',
  'e-csv-col-bmi':'ІМТ',
  'e-csv-col-norm':'Особиста норма тиску',
  'e-csv-col-std':'Стандарт',
  'e-csv-col-period':'Період',
  'e-csv-mmhg':'мм рт.ст.',
  'e-csv-cm':'см',
  'e-csv-kg':'кг',
  'e-csv-col-drug':'Препарат',
  'e-csv-col-dose':'Доза',
  'e-csv-col-time-med':'Час прийому',
  'e-csv-col-sched':'Розклад',
  'e-csv-col-adher-pct':'Виконання %',
  'e-csv-unknown':'Невідомий',
  'e-toast-csv-error':'Помилка генерації CSV',
  't-exp-from-lbl':'З дати',
  't-exp-to-lbl':'По дату',
  't-exp-print-btn':'Друк / PDF',
  't-exp-csv-btn':'Зберегти CSV',
  't-personal-norm-box':'💡 <b>Особиста норма</b> — допомагає визначати, чи завищений тиск саме для вас. Враховується у вимірюваннях, аналітиці та звітах для лікаря.',
  't-contacts-title':'Контактна інформація',
  't-normal-sys':'Звичайний тиск (сис.)',
  't-normal-dia':'Звичайний тиск (діа.)',
  't-normal-pulse':'Звичайний пульс (уд/хв)',
  't-journal-print':'Друк',
  't-profile-card':'Профіль користувача',
  't-sub-sys':'сис.','t-sub-dia':'діас.','t-sub-pulse':'уд/хв',
  't-leg-pulse':'Пульс',
  't-score-no-data':'Немає даних','t-score-add-more':'Додайте 3+ виміри для аналізу','t-score-bp-target':'Тиск',
  'p-sys':'120','p-dia':'80','p-pulse':'72','p-note':'після їжі, стрес...',
  'p-pillName':'Еналаприл','p-pillDose':'5 мг',
  'p-userName':'Ваше ім\'я','p-userAge':'60','p-userHeight':'170','p-userWeight':'75',
  'p-normalSys':'120','p-normalDia':'80','p-normalPulse':'70',
  'p-userPhone':'+380XXXXXXXXX','p-userEmail':'your@email.com',
  'p-userViber':'+380XXXXXXXXX','p-userTelegram':'@username або +380...','p-userWhatsapp':'+380XXXXXXXXX',
  'p-emergencyPhone':'+380XXXXXXXXX','p-emergencyName':'Лікар / Родич',

  // ─── meds ───
  'm-day-daily':'Щодня','m-day-date':'Дата',
  'm-day-weekdays':'Пн–Пт','m-day-mon':'Понеділок','m-day-tue':'Вівторок','m-day-wed':'Середа','m-day-thu':'Четвер','m-day-fri':'П\'ятниця','m-day-sat':'Субота','m-day-sun':'Неділя',
  'm-toast-need-name':'⚠️ Введіть назву препарату',
  'm-toast-need-dose':'⚠️ Введіть дозування (обов\'язково)',
  'm-toast-need-date':'⚠️ Виберіть дату прийому',
  'm-toast-past-date':'⚠️ Дата не може бути в минулому',
  'm-toast-added':'💊 {name} додано!',
  'm-toast-need-name-search':'⚠️ Введіть назву препарату для пошуку',
  'm-empty-no-today':'Ліків на сьогодні немає',
  'm-empty-no-pills':'Немає доданих ліків',
  'm-toast-taken':'✅ {name} прийнято!',
  'm-confirm-delete':'Видалити препарат?',
  'm-toast-deleted':'🗑 Видалено',
  'm-warn-max':'Добова доза — макс. <strong>{max} {unit}</strong>. {warn}.',
  'm-warn-ref':'Довідник',
  'm-validate-extreme':'⛔ Введена доза {num} мг здається помилкою. Перевірте: 5000+ мг будь-якого препарату є небезпечним!',
  'm-validate-danger':'⛔ НЕБЕЗПЕЧНА доза! Введено {num} {unit} — максимум {max} {unit}/добу! Це може призвести до отруєння.',
  'm-validate-warn':'⚠️ Введена доза ({num}) перевищує максимальну добову ({max} {unit}). Уточніть у лікаря.',
  'm-confirm-continue':'Продовжити все одно?',
  'm-summary-no-today':'Всього препаратів: {n}. На сьогодні немає.',
  'm-pill-max':'Макс: {max} {unit}/добу · {warn}',
  'm-overdue':'Пропущено',
  'm-past-badge':'минула',

  // ─── history ───
  'h-empty':'Немає записів за цей період',
  'h-aria-delete':'Видалити',
  'h-confirm-delete':'Видалити цей запис?',
  'h-toast-deleted':'🗑 Запис видалено',
  'h-confirm-clear':'Видалити всі виміри?',
  'h-toast-cleared':'🗑 Очищено',

  // ─── export ───
  'e-toast-json-saved':'✅ JSON збережено',
  'e-toast-no-data':'⚠️ Немає даних',
  'e-toast-no-data-period':'⚠️ Немає даних за вибраний період',
  'e-csv-col-date':'Дата','e-csv-col-time':'Час','e-csv-col-sys':'Систол.','e-csv-col-dia':'Діастол.','e-csv-col-pulse':'Пульс','e-csv-col-status':'Статус','e-csv-col-note':'Нотатка',
  'e-toast-csv-saved':'📊 CSV збережено',
  'e-toast-csv-error':'❌ Помилка експорту CSV',
  'e-toast-import-error':'❌ Помилка: {msg}',
  'e-pdf-prep':'⏳ Формування звіту для лікаря…',
  'e-pdf-saved':'Звіт збережено: {fname}',
  'e-pdf-error':'Помилка генерації PDF: {msg}',
  'e-pdf-page-lbl':'HealthPro v4.0 · {date} · Стор. {n} · Не є медичним діагнозом',
  'e-modal-empty':'Немає вимірів за вибраний період',
  'e-modal-w-1':'вимір','e-modal-w-few':'виміри','e-modal-w-many':'вимірів',
  'e-modal-count':'{n} {word} за вибраний період',
  'e-prep-pdf':'📄 Готую PDF…',

  // ─── pressure ───
  'pr-toast-need-bp':'Введіть систолічний та діастолічний тиск',
  'pr-toast-bad-sys':'Перевірте значення систолічного тиску',
  'pr-toast-bad-dia':'Перевірте значення діастолічного тиску',
  'pr-toast-bad-pulse':'Неприпустиме значення пульсу (допустимо від 30 до 250)',
  'pr-toast-saved':'Вимір збережено!',
  'pr-bpm-short':'уд/хв',
  'pr-mmhg':'мм рт.ст.',

  // ─── critical ───
  'cr-text':'Критичний тиск: {sys}/{dia}! Негайно зверніться до лікаря або викличте {phone}.',
  'cr-notify-title':'🚨 Критичний тиск!',
  'cr-notify-body':'Тиск {sys}/{dia} — виклич {phone}!',
  'cr-sms-body':'{name}! Критичний тиск {sys}/{dia} о {time}. HealthPro.',
  'cr-toast-no-phone':'⚠️ Вкажіть телефон у Профілі',
  'cr-toast-no-num':'⚠️ Введіть номер',
  'cr-sms-default-name':'Увага',

  // ─── norm ───
  'n-personal':'особиста норма {sys}/{dia}',
  'n-age-under18':'норми до 17 р.',
  'n-age-18-59':'норми 18–59 р.',
  'n-age-60-79':'норми 60–79 р.',
  'n-age-80plus':'норми 80+ р.',
  'n-bp-very-low':'⬇️ Дуже низький',
  'n-bp-low':'⬇️ Знижений тиск',
  'n-bp-normal':'✓ Норма',
  'n-bp-high':'⚠ Підвищений',
  'n-bp-ht1':'⚠ Гіпертензія І',
  'n-bp-ht2':'▲ Гіпертензія ІІ',
  'n-bp-crisis':'🚨 Криз!',
  // AHA 2017 категорії
  'n-bp-aha-elevated':'⚠ Підвищений (AHA)',
  'n-bp-aha-ht1':'⚠ HTN Ст. 1 (AHA)',
  'n-bp-aha-ht2':'▲ HTN Ст. 2 (AHA)',
  // IZ modal / scatter modal
  't-iz-trend-modal-title':'Тренд Індексу Здоров\'я',
  't-scatter-modal-title':'Кроки ↔ Тиск',
  't-iz-tap':'▲ Натисни — детальний графік',
  't-scatter-tap':'▲ Натисни — кореляційний графік',
  'n-pulse-brady':'Брадикардія',
  'n-pulse-low':'Знижений',
  'n-pulse-normal':'Норма',
  'n-pulse-high':'Підвищений',
  'n-pulse-tachy':'Тахікардія',

  // ─── who modal ───
  'w-cat-optimal':'Оптимальний','w-cat-normal':'Нормальний','w-cat-high-normal':'Високо-нормальний',
  'w-cat-ht1':'Гіпертензія І ст.','w-cat-ht2':'Гіпертензія ІІ ст.','w-cat-ht3':'Гіпертензія ІІІ ст.',
  'w-toast-no-data':'⚠️ Немає вимірів','w-recos':'Рекомендації:','w-links':'Посилання (🇺🇦 Ukraine):',

  // ─── analytics ───
  'a-score-no-data':'Немає даних',
  'a-score-excellent':'Відмінно','a-score-good':'Добре','a-score-fair':'Задовільно','a-score-poor':'Потребує уваги',
  'a-score-add-3':'Додайте 3+ виміри',
  'a-score-desc-good':'Показники в нормі','a-score-desc-fair':'Більшість показників добрі','a-score-desc-poor':'Зверніть увагу на рекомендації','a-score-desc-bad':'Рекомендується консультація лікаря',
  'a-target-bp':'Ціль тиску <{val}',
  'a-bd-bp':'Тиск','a-bd-pulse':'Пульс','a-bd-pills':'Ліки','a-bd-bmi':'ІМТ','a-bd-activity':'Активність',
  'a-no-data':'немає даних',
  'a-who-current':'На основі останнього виміру',
  'rec-ok-personal-h':'Тиск відповідає вашій нормі',
  'rec-ok-personal-s':'{bp} · особиста норма: {norm} · за {std}: {status}',
  'hs-personal-note':'ℹ Розрахунок за особистою нормою. За {std}: {status}',
  'a-trend-stable':'→ Стабільний','a-trend-down':'↓ Знижується','a-trend-up':'↑ Зростає',
  'a-trend-stable-sub':'Без суттєвих змін','a-trend-down-sub':'На {n} мм нижче','a-trend-up-sub':'На {n} мм вище',
  'a-trend-need-5':'потрібно 5+ вимірів','a-who-tap':' → Натисни для деталей',
  'a-week-prefix':'Тиждень: {n} · Журнал →','a-min-prefix':'мін.',
  'a-trend-avg-14':'Середній 7 вим.','a-trend-status':'Статус',

  // ─── bmi ───
  'b-cat-very-low':'Виражений дефіцит','b-impact-very-low':'Ризик гіпотонії та серцевих проблем',
  'b-cat-low':'Дефіцит маси','b-impact-low':'Можлива гіпотонія',
  'b-cat-normal':'Норма ✓','b-impact-normal':'Нейтральний вплив на тиск',
  'b-cat-over':'Надмірна вага','b-impact-over':'Ризик гіпертонії +15–20%',
  'b-cat-ob1':'Ожиріння І ст.','b-impact-ob1':'Кожен +5 кг ≈ +2–3 мм рт.ст.',
  'b-cat-ob2':'Ожиріння ІІ ст.','b-impact-ob2':'Серйозний ризик гіпертензії',
  'b-cat-ob3':'Ожиріння ІІІ ст.','b-impact-ob3':'Критичний ризик!',
  'b-ideal-weight':'Ідеальна вага','b-height':'Зріст','b-weight':'Вага',
  'b-zone-deficit':'Дефіцит','b-zone-norm':'Норма','b-zone-excess':'Надлишок','b-zone-obese':'Ожиріння',
  'b-impact-title':'Вплив на тиск',
  'b-tip-lose':'Зниження ваги на 5–10 кг може знизити систолічний тиск на 5–10 мм рт.ст.',
  'b-tip-norm':'ІМТ в нормі — позитивний вплив на контроль тиску.',
  'b-empty-fill-profile':'Заповніть зріст та вагу в Профілі',
  'b-imt':'ІМТ','b-cm':'см','b-kg':'кг',
  'b-age-note-65':'Норми ІМТ скориговані для 65+ (22–27)',

  // ─── chart tooltips ───
  'c-no-data':'Недостатньо даних',
  'c-tt-sys':'Сис.: ','c-tt-dia':'Діас.: ','c-tt-pulse':'Пульс: ',

  // ─── settings/disclaimer ───
  'd-accepted':'Прийнято','d-version':'Версія','d-history':'Історія погоджень',
  'd-warn-new':'⚠ Доступна нова версія дисклеймера. Будь ласка, ознайомтеся.',

  // ─── settings/notifications ───
  'nt-no-support':'❌ Браузер не підтримує сповіщення',
  'nt-on':'🔔 Сповіщення увімкнено!',
  'nt-test-body':'Сповіщення налаштовані!',
  'nt-denied':'❌ Дозвіл відхилено',
  'nt-off':'🔕 Вимкнено',
  'nt-meas-on':'🔔 Нагадування про вимір увімкнено!',
  'nt-time-saved':'✅ Час збережено',
  'nt-pill-time':'Час прийому ',
  'nt-pill-toast':'💊 Час прийняти: ',
  'nt-bp-title':'🩺 Вимір тиску',
  'nt-bp-body':'Час для щоденного виміру тиску',
  'nt-bp-toast':'🩺 Час для виміру тиску',
  'nt-perm-title':'Дозвольте сповіщення?',
  'nt-perm-body':'HealthPro може нагадувати про прийом ліків та щоденні виміри тиску. Ви можете змінити це у Налаштуваннях.',
  'nt-perm-yes':'Так, дозволити','nt-perm-no':'Ні, дякую',
  'nt-perm-warn':'Сповіщення вимкнено. Ви можете увімкнути їх у Налаштуваннях.',

  // ─── settings/profile ───
  'pf-saved':'✅ Профіль збережено!',

  // ─── steps ───
  'st-no-perm':'❌ Немає дозволу на акселерометр',
  'st-no-sensor':'⚠️ Акселерометр не знайдено. Крокомір працює лише на мобільному пристрої.',
  'st-off':'🦶 Лічильник вимкнено',
  'st-on':'🦶 Лічильник кроків увімкнено!',
  'st-goal-pref':'ціль:',

  // ─── step permission modal (Modal A) ───
  'st-perm-title':'Відстеження активності',
  'st-perm-body':'HealthPro хоче рахувати ваші кроки через сенсор телефону. Дані зберігаються лише на пристрої та не передаються нікому.',
  'st-perm-yes':'Так, дозволити',
  'st-perm-no':'Ні, дякую',
  'st-perm-denied':'❌ Дозвіл відхилено. Увімкніть у налаштуваннях додатку.',
  'st-open-settings-confirm':'Доступ до активності заборонено. Відкрити налаштування додатку, щоб увімкнути?',

  // ─── foreground service consent modal (Modal B) ───
  'st-fg-title':'Крокомір у фоновому режимі',
  'st-fg-body':'Для точного підрахунку кроків навіть коли додаток закритий, HealthPro запустить фоновий сервіс. Ви побачите постійне сповіщення з кількістю кроків. Споживання батареї: +1–3% на день.',
  'st-fg-yes':'Запустити у фоні',
  'st-fg-no':'Тільки при відкритому додатку',

  // ─── step mode toasts ───
  'st-mode-fg':'Фоновий крокомір увімкнено!',
  'st-mode-active':'Крокомір (тільки активний режим) увімкнено!',
  'st-service-restored':'Крокомір відновлено після збою системи.',
  'st-battery-opt':'⚠️ Увімкніть «Без обмежень» у налаштуваннях батареї, щоб крокомір не вимикався у фоні.',

  // ─── foreground service notification texts ───
  'st-notif-title':'HealthPro · Кроки',
  'st-notif-text':'Відстеження активності...',

  // ─── app.js ───
  'app-step-sub':'Через сенсори телефону.',

  // ─── settings/profile ───
  'profile-toast-saved':'✅ Профіль збережено!',

  // ─── settings/disclaimer ───
  'discl-accepted':'Прийнято',
  'discl-version':'Версія',
  'discl-history':'Історія погоджень',
  'discl-warn-new':'⚠ Доступна нова версія дисклеймера. Будь ласка, ознайомтеся.',

  // ─── settings/notifications ───
  'notif-no-support':'❌ Браузер не підтримує сповіщення',
  'notif-on':'🔔 Сповіщення увімкнено!',
  'notif-confirm-title':'✅ HealthPro',
  'notif-confirm-body':'Сповіщення налаштовані!',
  'notif-denied':'❌ Дозвіл відхилено',
  'notif-off':'🔕 Вимкнено',
  'notif-measure-on':'🔔 Нагадування про вимір увімкнено!',
  'notif-time-saved':'✅ Час збережено',
  'notif-pill-time':'Час прийому ',
  'notif-pill-toast':'💊 Час прийняти: ',
  'notif-bp-title':'🩺 Вимір тиску',
  'notif-bp-body':'Час для щоденного виміру тиску',
  'notif-bp-toast':'🩺 Час для виміру тиску',
  'notif-pill-title':'💊 Час прийняти: {name}',
  'notif-pill-body':'Натисніть, щоб відмітити як прийнятий',
  'notif-pill-on':'💊 Нагадування про ліки увімкнено!',
  'notif-open-settings-confirm':'Сповіщення заборонені. Відкрити налаштування додатку, щоб увімкнути?',
  'notif-fcm-soon':'FCM push — скоро',
  'notif-set-times-first':'Спочатку задайте час Ранок/Вечір',
  'notif-exact-alarm-needed':'Дозвольте “Будильники й нагадування”, потім увімкніть нагадування ще раз.',


  // ─── notif permission modal (post-disclaimer) — IDs match HTML ───
  'nt-perm-title':'Увімкнути нагадування?',
  'nt-perm-body':'Дозвольте сповіщення, щоб отримувати нагадування про прийом ліків та вимір тиску.',
  'nt-perm-no':'Не зараз',
  'nt-perm-yes':'Дозволити',
  'notif-perm-warn':'⚠ Сповіщення вимкнено. Можна увімкнути в Налаштуваннях.',

  // ─── Email/SMS reminders (#11) ───
  't-send-email':'Надіслати Email',
  't-send-sms':'Надіслати SMS',
  'notif-email-subject':'HealthPro: нагадування',
  'notif-email-body':'Час виміряти тиск! HealthPro нагадує.',
  'notif-sms-body':'HealthPro: час виміряти тиск!',
  'notif-no-email':'⚠ Заповніть Email у Профілі',
  'notif-no-phone':'⚠ Заповніть телефон у Профілі',
  'notif-targets-empty':'Адреса/телефон не заповнені (відкрийте Профіль).',
  'notif-targets-email':'Email:',
  'notif-targets-phone':'Тел.:',

  // ─── settings/data ───
  'data-clear-confirm':'Видалити ВСІ дані? Дію скасувати неможливо.',

  // ─── critical (SMS) ───
  'cr-emergency-name-default':'Увага',
  'cr-test-sms-body':'Тест HealthPro — перевірка зв\'язку. Все гаразд!',

  // ─── health-score veto / norm labels ───
  'hs-veto-crisis':'🚨 Гіпертонічний криз — штраф ×0.30',
  'hs-veto-ht2':'⚠️ Гіпертонія 2 ст. — штраф ×0.60',
  'hs-veto-hypo':'⬇️ Гіпотонія — штраф ×0.55',
  'hs-veto-default':'⚠️ Вето застосовано',

  // ─── v5.1: scatter / bp-zones / tips / journal ───
  't-scatter-title':'Кроки ↔ Тиск — кореляція',
  't-scatter-empty':'Потрібно 10+ парних вимірювань кроків та тиску',
  't-scatter-x':'Кроки',
  't-scatter-y':'Сист. тиск (мм рт.ст.)',
  't-bp-zones-title':'Розподіл по зонах ВООЗ',
  'tips-title':'Поради ESC 2024',
  'tips-title-aha':'Поради AHA 2017',
  'tips-empty':'Додайте 3+ виміри для персональних рекомендацій',
  'tips-disclaimer':'Інформаційний характер. Зверніться до лікаря.',
  't-btn-iz-trend':'Тренд 30 днів',
  't-btn-who-class':'Класифікація',
  't-btn-bp-zones':'Розподіл по зонах',
  't-btn-scatter':'Кроки ↔ Тиск',
  't-btn-steps-day':'Кроки по днях',
  't-steps-day-modal-title':'Кроки по днях',
  't-steps-day-empty':'Додайте 2+ дні кроків для графіка',
  'j-filter-all':'Всі',
  'j-filter-pressure':'Тиск',
  'j-date-from':'З дати',
  'j-date-to':'По дату',
  'j-no-data':'Немає записів за вибраний період',
  'j-delete':'Видалити запис',
  'hs-norm-personal':'👤 Особиста норма',
  'hs-norm-standard':'📋 Стандартна норма',

  // ─── bp standard switcher ───
  't-bp-std-title':'Стандарт тиску',
  't-bp-std-label':'Стандарт класифікації',
  't-bp-std-sub':'ESC 2024 або AHA 2017',
  't-bp-std-info':'ESC 2024 — Євростандарт. AHA 2017 — американський стандарт.',

  // ─── meds modal disclaimer ───
  'm-warn-disclaimer':'Самолікування небезпечне. Завжди консультуйтеся з лікарем.',
  'm-discl-title':'Застереження щодо ліків',
  'm-discl-text':'Інформація про препарати носить виключно інформаційний характер. Не є медичною порадою. Перед застосуванням будь-якого препарату проконсультуйтеся з лікарем. Самолікування може бути небезпечним для вашого здоров\'я.',

  // ─── v5.2 Part 2: нові ключі ───
  // Tabletki.ua (Task 1)
  'm-search-source':'Пошук: Tabletki.ua',

  // Біометрія (Task 2)
  's-biometric':'Блокування PIN-кодом',
  's-biometric-sub':'4-значний код для входу',
  's-biometric-reason':'Підтвердіть особу для доступу до HealthPro',
  's-biometric-cancel':'Скасувати',
  's-pin-lock':'Блокування PIN-кодом',
  's-pin-lock-sub':'4-значний код для входу',
  't-lock-title':'HealthPro заблоковано',
  't-lock-sub':'Введіть PIN-код',
  't-lock-btn':'Розблокувати',

  // Adherence chart (Task 3)
  't-adherence-title':'Прийом ліків — 30 днів',
  't-adherence-empty':'Потрібно 3+ дні прийому ліків для графіка',
  't-btn-adherence':'Графік прийому',

  // PDF-звіт для лікаря (Task 5)
  'export-doctor-report':'Звіт для лікаря (PDF)',
  'export-doctor-report-sub':'Тиск, ліки, ІЗ — повний звіт',

  // Дні прийому (Task 6)
  'm-day-mon-wed-fri':'Пн/Ср/Пт',
  'm-day-tue-thu-sat-sun':'Вт/Чт/Сб/Нд',

  // ─── v5.3: Бекап (.hpb) та виправлення біометрії ───
  'bk-export-lbl':'Зберегти бекап',
  'bk-export-sub':'Зашифрований .hpb — всі дані',
  'bk-import-lbl':'Відновити бекап',
  'bk-import-sub':'Відновити з .hpb або .json файлу',
  'bk-export-modal-title':'Зберегти бекап',
  'bk-password-lbl':'Пароль',
  'bk-password-confirm-lbl':'Підтвердити пароль',
  'bk-password-hint':'Запам\'ятайте пароль — без нього відновлення неможливе!',
  'bk-do-export-btn':'Зберегти бекап',
  'bk-import-modal-title':'Введіть пароль бекапу',
  'bk-do-decrypt-btn':'Розшифрувати',
  'bk-confirm-title':'Відновити дані?',
  'bk-confirm-warn':'Поточні дані буде замінено. Цю дію неможливо скасувати.',
  'bk-confirm-biometric':'PIN-блокування буде вимкнено — увімкніть у Налаштуваннях після відновлення.',
  'bk-do-restore-btn':'Відновити дані',
  'bk-cancel':'Скасувати',
  'bk-stats-m':'Вимірів',
  'bk-stats-med':'Ліків',
  'bk-stats-steps':'Днів кроків',
  'bk-stats-date':'Дата бекапу',
  'bk-err-format':'Невірний формат файлу',
  'bk-err-password':'Невірний пароль',
  'bk-err-checksum':'Файл пошкоджений або змінений',
  'bk-err-passwords-match':'Паролі не збігаються',
  'bk-err-empty-password':'Введіть пароль',
  'bk-toast-saved':'Бекап збережено',
  'bk-toast-restored':'Дані відновлено! Блокування вимкнено — увімкніть у Налаштуваннях.',
  'bk-no-password':'Без пароля (незашифрований)',
  'bk-use-password-lbl':'Зашифрувати паролем',
  'bk-err-export':'Помилка збереження бекапу',
  'bk-err-restore':'Помилка відновлення',
  'bio-err-unavailable':'Налаштуйте PIN або відбиток пальця у системі Android',

  // ─── Біометрія — відбиток поверх PIN ───
  'bio-toggle':      'Розблокування відбитком',
  'bio-toggle-hint': 'Потребує встановленого PIN',
  'lock-bio-btn':    'Увійти відбитком пальця',

  // ─── step averages ───
  'st-week-avg': 'Середнє за тиждень',
  'st-month-avg': 'Середнє за місяць',

  // ─── auto-backup reminder ───
  'bk-auto-remind': '📦 Пройшло 7+ днів без бекапу. Збережіть копію у Профіль → Резервна копія.',

  // ─── WHO-2: BP стандарти modal ───
  't-bp-std-modal-title':'Стандарти класифікації тиску',
  't-bp-std-modal-sub':'Різниця між ESC 2024 та AHA 2017',
  't-bp-std-col-cat':'Категорія',
  't-bp-std-r-opt':'Оптимальний',
  't-bp-std-r-norm':'Нормальний',
  't-bp-std-r-highnorm':'Висока норма',
  't-bp-std-r-ht1':'Гіпертензія 1',
  't-bp-std-r-ht2':'Гіпертензія 2',
  't-bp-std-r-crisis':'Гіпертонічний криз',
  't-bp-std-why-title':'Чому ESC не прийняв пороги AHA?',
  't-bp-std-why-text':'ESC 2024 зберіг поріг 140/90 через ефект "білого халата" та різниці в даних. AHA 2017 знизила поріг до 130/80 за американськими когортами.',
  't-bp-std-home-title':'Домашні виміри — норма нижча',
  't-bp-std-home-text':'Поріг гіпертензії при домашньому вимірі: ≥135/85 мм рт. ст. (замість ≥140/90 у клініці). Якщо вдома тиск ≥135/85 — це вже гіпертензія, навіть якщо в клініці було б «норма».',
  't-bp-std-rec-title':'Що обрати?',
  't-bp-std-rec-text':'ESC 2024 — для України та Європи. AHA 2017 — якщо ваш лікар орієнтується на американські джерела.',
  't-bp-std-sources':'Джерела: ESC/ESH 2024 · AHA/ACC 2017',
  't-bp-std-ok':'Зрозуміло',
  // ─── PDF-2: секції звіту ───
  't-exp-sections-lbl':'Розділи звіту',
  't-exp-s-chart':'Графік тиску',
  't-exp-s-journal':'Журнал вимірів',
  't-exp-s-meds':'Ліки',
  't-exp-s-adherence':'Прийом ліків',
  't-exp-s-doctor':'Блок лікаря',
  't-exp-pdf-doctor-btn':'PDF для лікаря',
  // ─── PDF-1/3: toast messages ───
  'e-doctor-pdf-prep':'Генерація PDF — зачекайте...',
  'e-doctor-pdf-done':'✓ Звіт готовий!',
  'e-doctor-pdf-error':'Помилка генерації PDF',
  // ─── PIN lock (П1) ───
  'pin-setup-title':'Встановити PIN-код',
  'pin-setup-step1':'Введіть 4-значний PIN-код',
  'pin-setup-step2':'Підтвердіть PIN-код',
  'pin-wrong':'Невірний PIN. Спробуйте ще раз.',
  'pin-mismatch':'PIN-коди не збігаються. Спробуйте знову.',
  'pin-set-ok':'PIN-код встановлено',
  'pin-disabled':'Блокування вимкнено',
};
