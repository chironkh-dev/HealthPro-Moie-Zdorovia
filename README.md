...
Усі медичні дані зберігаються **виключно локально на вашому пристрої** у SQLite базі. Жодних серверів, жодного хмарного зберігання, жодного стеження.
...
| 🔒 **PIN-блокування** | ✅ Готово | SHA-256 PIN, блокування при поверненні з фону (onResume) |
| 👆 **Біометрія** | 🔧 У розробці | Відбиток пальця (@capgo/capacitor-native-biometric), fallback на PIN |
...
| 🔐 **SQLCipher** | ⏳ Планується | Шифрування SQLite at-rest + Android Keystore |
...
| PIN-блокування | ✅ | SHA-256(salt+pin) в localStorage, блокування при onResume |
| Біометрія | 🔧 У розробці | `@capgo/capacitor-native-biometric` v8.4.2, fallback на PIN |
| Шифрування at-rest | ⏳ Планується | SQLCipher (`@capacitor-community/sqlite` encryption: true) |
| Ключ шифрування | ⏳ Планується | Android Keystore / iOS Keychain |
...
- [ ] **Біометрія** — відбиток пальця, фінальний фікс
- [ ] **Авто-блокування** — після 5 хв у фоні (Б5)

### v5.4 — Аналітика і звіти
- [ ] **SQLCipher** — шифрування SQLite at-rest
- [ ] **ScatterChart** «Кроки ↔ Тиск»
- [ ] **BarChart** розподіл по зонах ВООЗ
- [ ] **Adherence-трекер** ліків
- [ ] **PDF-звіт для лікаря** з ECharts-графіками
- [ ] **Офлайн поради** — `assets/tips/tips_uk.json`
